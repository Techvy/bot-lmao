...
class Moderation(commands.Cog, name="moderation"):
    ...
    @app_commands.describe(seconds="The number of seconds for the slowmode delay.")
    async def slowmode(self, context: Context, seconds: int) -> None:
        """
        Set the slowmode delay for the current channel.

        :param context: The hybrid command context.
        :param seconds: The number of seconds for the slowmode delay.
        """
        ...
        try:
            ...
            embed = discord.Embed(
                description=f"Slowmode delay set to {seconds} seconds.",
                color=0xBEBEFE,
            )
            embed.set_footer(text=f"Requested by {context.author}", icon_url=f"{context.author.display_avatar.url}?size=128")
            await context.send(embed=embed)
            ...
            embed = discord.Embed(
                description="An error occurred while trying to set the slowmode delay.",
                color=0xE02B2B,
            )
            embed.set_footer(text=f"Requested by {context.author}", icon_url=f"{context.author.display_avatar.url}?size=128")
            await context.send()
    ...
    @commands.hybrid_command(
        name="noslowmode",
        description="Remove the slowmode delay for the current channel.",
    )
    @commands.has_permissions(manage_channels=True)
    @commands.bot_has_permissions(manage_channels=True)
    async def noslowmode(self, context: Context) -> None:
        """
        Remove the slowmode delay for the current channel.
        ...
        try:
            ...
            embed = discord.Embed(
                description="Slowmode delay removed.",
                color=0xBEBEFE,
            )
            embed.set_footer(text=f"Requested by {context.author}", icon_url=f"{context.author.display_avatar.url}?size=128")
            await context.send(embed=embed)
            ...
            embed = discord.Embed(
                description="An error occurred while trying to remove the slowmode delay.",
                color=0xE02B2B,
            )
            embed.set_footer(text=f"Requested by {context.author}", icon_url=f"{context.author.display_avatar.url}?size=128")
            await context.send(embed=embed)
...
#unnick
    ...
    @commands.hybrid_command(
        name="unnick",
        description="Reset the nickname of a user on a server.",
    )
    @commands.guild_only()
    @commands.has_permissions(manage_nicknames=True)
    async def unnick(self, ctx, member: discord.Member = None):
        if member is None:
            member = Context.author  # This line should be here
        else:
            pass  # You can add additional logic here if needed
        await member.edit(nick=None)
        embed = discord.Embed(
            description=f"**{member}'s** nickname has been reset!",
            color=0xBEBEFE,
        )
        embed.set_author(name=member.display_name, icon_url=member.display_avatar.url)
        embed.set_footer(text=f"Requested by {ctx.author}", icon_url=f"{ctx.author.display_avatar.url}?size=128")
...
#ban
    ...
    async def ban(
        self, context: commands.Context, user: discord.User, *, reason: str = "Not specified"
    ) -> None:
        """
...
#Lock channel
    ...
    @commands.has_permissions(manage_channels=True)
    @commands.bot_has_permissions(manage_channels=True)
    @app_commands.describe(channel="The channel to lock. If not specified, the current channel will be locked.")
    async def lock(self, context: Context, channel: discord.TextChannel = None) -> None:
        """
        Lock a channel.

        :param context: The hybrid command context.
        :param channel: The channel to lock. If not specified, the current channel will be locked.
        """
        if not context.author.guild_permissions.administrator:
            embed = discord.Embed(
                description="You do not have administrator permissions.",
        ...
        try:
            ...
            embed = discord.Embed(
                description=f"Channel {channel.mention} locked.",
                color=0xBEBEFE,
            )
            await context.send(embed=embed)
        except:
            embed = discord.Embed(
                description=f"An error occurred while trying to lock {channel.mention}.",
                color=0xE02B2B,
            )
            await context.send(embed=embed)
...
#Unlock channel
    ...
    @commands.has_permissions(manage_channels=True)
    @commands.bot_has_permissions(manage_channels=True)
    @app_commands.describe(channel="The channel to unlock. If not specified, the current channel will be unlocked.")
    async def unlock(self, context: Context, channel: discord.TextChannel = None) -> None:
        """
        Unlock a channel.

        :param context: The hybrid command context.
        :param channel: The channel to unlock. If not specified, the current channel will be unlocked.
        """
        if not context.author.guild_permissions.administrator:
            embed = discord.Embed(
                description="You do not have administrator permissions.",
        ...
        try:
            ...
            embed = discord.Embed(
                description=f"Channel {channel.mention} unlocked.",
                color=0xBEBEFE,
            )
            await context.send(embed=embed)
        except:
            embed = discord.Embed(
                description=f"An error occurred while trying to unlock {channel.mention}.",
                color=0xE02B2B,
            )
            await context.send(embed=embed)
...
# AUDIT LOG
    @commands.hybrid_command(
        name="auditlog",
        description="Retrieves and displays audit log entries.",
    )
    @commands.has_permissions(view_audit_log=True)
    @app_commands.describe(
        limit="The limit of audit log entries to retrieve.",
    )
    async def auditlog(self, context: commands.Context, limit: int = 5) -> None:
        """
        Retrieves and displays audit log entries.

        :param context: The hybrid command context.
        :param limit: The limit of audit log entries to retrieve. Default is 5.
        """
        try:
            ...
            async for entry in context.guild.audit_logs(limit=limit):
                audit_log_embed.add_field(
                    name=f"Audit Log Entry",
                    value=f"Action: {entry.action}\n"
                        f"User: {entry.user.mention}\n"
                        f"Target: {entry.target}\n"
                        f"Reason: {entry.reason if entry.reason else 'Not specified'}\n"
            ...
            await dm_channel.send(embed=audit_log_embed)

            embed = discord.Embed(description=f"The audit logs have been passed to your dms. \n Command used by: {context.author.mention}", color=0x00ff00)
            ...
            embed = discord.Embed(
                description="I do not have permission to view audit logs.",
                color=0xE02B2B
            )
            await context.send(embed=embed)
...
#Mute And Unmute Members
    ...
    async def mute(self, context: discord.Interaction, member: discord.Member) -> None:
        ...
        embed = discord.Embed(title="Mute", description=f"{member.mention} has been permanently muted.", color=0x00ff00)
    ...
    async def unmute(self, context: discord.Interaction, member: discord.Member) -> None:
        mute_role = discord.utils.get(context.guild.roles, name="Muted")
        ...
        if mute_role in member.roles:
            ...
            embed = discord.Embed(title="Unmute", description=f"{member.mention} is not muted.", color=0xff0000)
            embed.set_footer(text=f"Requested by {context.author}", icon_url=f"{context.author.display_avatar.url}?size=128")
            await context.send(embed=embed)
